...
logger.info(
    f"Loaded PAPERLESS_API_TOKEN: {'[SET]' if os.getenv('PAPERLESS_API_TOKEN') else '[NOT SET]'}"
)
...
class GlobalState:
    def __init__(self):
        ...
        self._indexed_document_ids = (
            set()
        )  # Store temporarily until data_manager is initialized
        ...
        try:
            if os.path.exists(STATE_FILE):
                ...
                if schema_version != self.state_schema_version:
                    logger.warning(
                        f"State file schema version mismatch: {schema_version} vs {self.state_schema_version}"
                    )
                ...
                if "indexing_status" in state_dict:
                    ...
                    logger.info(
                        f"Loaded indexing status: {self.indexing_status.documents_count} documents, last indexed: {self.indexing_status.last_indexed}"
                    )
                ...
                logger.info(
                    f"System state loaded from {STATE_FILE} with {len(self._indexed_document_ids)} indexed document IDs"
                )
                return True
            else:
...
class DataManager:
    def __init__(self, initialize_on_start=False):
        ...
        if paperless_api_url and paperless_api_url.endswith("/api"):
            paperless_api_url = paperless_api_url[
                :-4
            ]  # Entfernen der letzten 4 Zeichen (/api)
        ...
        logger.info(
            f"Environment variables: PAPERLESS_API_URL={os.getenv('PAPERLESS_API_URL')}, PAPERLESS_URL={os.getenv('PAPERLESS_URL')}, PAPERLESS_NGX_URL={os.getenv('PAPERLESS_NGX_URL')}, PAPERLESS_HOST={os.getenv('PAPERLESS_HOST')}"
        )
        logger.info(
            f"Environment variables: PAPERLESS_TOKEN={'[SET]' if os.getenv('PAPERLESS_TOKEN') else '[NOT SET]'}, PAPERLESS_API_TOKEN={'[SET]' if os.getenv('PAPERLESS_API_TOKEN') else '[NOT SET]'}, PAPERLESS_APIKEY={'[SET]' if os.getenv('PAPERLESS_APIKEY') else '[NOT SET]'}"
        )

        if not self.paperless_url or not self.paperless_token:
            logger.error(
                "Missing PAPERLESS_API_URL/PAPERLESS_URL or PAPERLESS_API_TOKEN in .env file"
            )
    ...
    def load_documents(self, force_refresh=False, check_new=False):
        ...
        if os.path.exists(DOCUMENTS_FILE) and not force_refresh:
            ...
            try:
                ...
                if not isinstance(local_documents, list) or (
                    local_documents and not isinstance(local_documents[0], dict)
                ):
                ...
                if not self.indexed_document_ids:
                    self.indexed_document_ids = {
                        doc["id"] for doc in local_documents if "id" in doc
                    }
                    logger.info(
                        f"Initialized indexed_document_ids with {len(self.indexed_document_ids)} document IDs"
                    )
            ...
            if force_refresh:
                ...
                self.new_document_ids = (
                    self.indexed_document_ids.copy()
                )  # All docs are "new" in a full refresh
                self.save_documents()
    ...
    def setup_chroma_collection(self, force_update=False):
        ...
        try:
            ...
            collection_exists = any(
                c.name == COLLECTION_NAME for c in existing_collections
            )
            ...
            if collection_exists and not force_update:
                try:
                    ...
                    logger.info(
                        f"ChromaDB collection contains {collection_count} documents"
                    )
                    ...
                    if (
                        ...
                        logger.warning(
                            "Empty ChromaDB collection found but documents exist. Forcing update."
                        )
                        force_update = True
                        ...
                        if self.new_document_ids and not force_update:
                            logger.info(
                                f"Adding {len(self.new_document_ids)} new documents to ChromaDB collection"
                            )
            ...
            if force_update and collection_exists:
                logger.info(
                    f"Forcing update of ChromaDB collection '{COLLECTION_NAME}'"
                )
    ...
    def _add_documents_to_chroma(self, collection, documents):
        ...
        for i in range(0, total_docs, batch_size):
            ...
            logger.info(
                f"Processing batch {i // batch_size + 1}/{(total_docs - 1) // batch_size + 1} ({len(batch)} documents)"
            )
...
class SearchEngine:
    ...
    def validate_state(self):
        ...
        if (
            ...
            if len(self.tokenized_corpus) != len(self.documents):
                logger.error(
                    f"BM25 corpus size mismatch: {len(self.tokenized_corpus)} vs {len(self.documents)} documents"
                )
                valid = False
            else:
                logger.info(
                    f"BM25 index contains {len(self.tokenized_corpus)} documents"
                )
    ...
    def _add_new_documents_to_bm25(self):
        ...
        try:
            logger.info(
                f"Adding {len(self.data_manager.new_document_ids)} new documents to BM25 index"
            )
            ...
            if new_docs_processed > 0:
                ...
                logger.info(
                    f"BM25 index updated with {new_docs_processed} new documents"
                )
            else:
        ...
        try:
            ...
            for i, doc_id in enumerate(results["ids"][0]):
                try:
                    ...
                    doc = next(
                        (d for d in self.documents if str(d["id"]) == doc_id), None
                    )
    ...
    def hybrid_search(self, query, top_k=MAX_RESULTS):
        ...
        if keyword_results:
            ...
            for r in keyword_results:
                r["score"] = (
                    r["score"] / max_keyword_score if max_keyword_score > 0 else 0
                )
        ...
        for result in keyword_results:
            doc_id = result["id"]
            results_map[doc_id] = {**result, "score": result["score"] * BM25_WEIGHT}
    ...
    def rerank_results(self, query, results, top_k=MAX_RESULTS):
        ...
        try:
            ...
            if not isinstance(cross_scores, np.ndarray) or len(cross_scores) != len(
                results
            ):
                logger.error(
...
def run_indexing(force_update=False, check_new=False):
    ...
    try:
        ...
        if (
            ...
            logger.warning(
                "Search engine validation failed after indexing, forcing rebuild"
            )
...
async def startup_event():
    ...
    try:
        ...
        logger.info(
            f"Loaded state has documents_count: {global_state.indexing_status.documents_count}"
        )

        # Überprüfen, ob .env-Datei existiert
        env_file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "data", ".env"
        )
        if not os.path.exists(env_file_path):
            ...
            logger.info(
                f"Created example .env file at {os.path.abspath(env_file_path)}"
            )
            logger.info(
                "Please edit the .env file with your Paperless-NGX API configuration"
            )
            logger.warning(
        ...
        if documents_exist:
                ...
                try:
                    ...
                    with open(DOCUMENTS_FILE, "r", encoding="utf-8") as f:
                        ...
                        if not isinstance(loaded_docs, list) or (
                            loaded_docs and not isinstance(loaded_docs[0], dict)
                        ):
                            logger.error("Invalid document structure in documents.json")
                            raise ValueError(
                    ...
                    global_state.data_manager.indexed_document_ids = set(
                        doc["id"] for doc in global_state.data_manager.documents
                    )
                    ...
                    if global_state.indexing_status.documents_count != doc_count:
                        logger.warning(
                            f"Fixing inconsistent document count: {global_state.indexing_status.documents_count} -> {doc_count}"
                        )
                        global_state.indexing_status.documents_count = doc_count
                ...
                if bm25_exists and global_state.search_engine:
                    try:
                        ...
                        if global_state.search_engine.tokenized_corpus and len(
                            ...
                            logger.warning(
                                f"BM25 corpus size mismatch: {len(global_state.search_engine.tokenized_corpus)} vs {len(global_state.data_manager.documents)} documents"
                            )
            ...
            if global_state.indexing_status.documents_count != doc_count:
                logger.warning(
                    f"Final fix for document count: {global_state.indexing_status.documents_count} -> {doc_count}"
                )
                global_state.indexing_status.documents_count = doc_count
        ...
        logger.info(
            f"Final documents_count in indexing_status: {global_state.indexing_status.documents_count}"
        )

    except Exception as e:
        logger.error(f"Error during startup: {str(e)}")
        logger.error(traceback.format_exc())
        global_state.system_status.server_up = (
            True  # Server läuft, aber mit Einschränkungen
        )
...
async def get_context(
    ...
    try:
        ...
        return {"context": context, "sources": sources, "query": request.question}
    except Exception as e:
...
async def get_status():
    ...
    if (
        ...
        if doc_count > 0 and global_state.indexing_status.documents_count != doc_count:
            logger.warning(
                f"Correcting document count discrepancy in status response: {global_state.indexing_status.documents_count} -> {doc_count}"
            )
            global_state.indexing_status.documents_count = doc_count
        ...
        logger.info(
            f"Preserving document count in status response: {global_state.indexing_status.documents_count}"
        )
    ...
    logger.info(
        f"Status API returning documents_count: {global_state.system_status.indexing_status.documents_count}"
    )
...
async def check_health():
    ...
    try:
        ...
        if global_state.data_manager and global_state.data_manager.documents:
            ...
            if doc_count == 0:
                ...
                health_status["recommendations"].append(
                    "Check Paperless API configuration and call /indexing/start with force=true"
                )
        else:
        ...
        if not global_state.search_engine:
            ...
            if global_state.search_engine.collection:
                ...
                try:
                    ...
                    if collection_count == 0:
                        ...
                        health_status["recommendations"].append(
                            "Call /indexing/start with force=true"
                        )
                except Exception as e:
                    health_status["issues"].append(f"ChromaDB error: {str(e)}")
                    health_status["recommendations"].append(
                        "Call /indexing/start with force=true to rebuild ChromaDB"
                    )
            else:
            ...
            # Check BM25
            if (
                global_state.search_engine.bm25
                ...
                if (
                    health_status["documents_loaded"]
                    and bm25_count != health_status["document_count"]
                ):
                    health_status["issues"].append(
                        f"BM25 document count mismatch: {bm25_count} vs {health_status['document_count']}"
                    )
                    health_status["recommendations"].append(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--force-refresh", action="store_true", help="Force refresh documents from API"
    )
    parser.add_argument(
        "--auto-load",
        action="store_true",
    ...
    parser.add_argument(
        "--check-new", action="store_true", help="Check for new documents on startup"
    )
    parser.add_argument(
        "--skip-check",
        action="store_true",
    ...
    if args.initialize:
        ...
        async def initialize_on_startup():
            ...
            if global_state.system_status.data_loaded and not args.force_refresh:
                logger.info(
                    f"Already have {global_state.indexing_status.documents_count} documents loaded"
                )
                ...
                if args.skip_check and not args.rebuild_indexes:
                    ...
                    if not global_state.system_status.index_ready:
                        ...
                        global_state.search_engine.initialize(
                            force_update=args.rebuild_indexes
                        )
